...
function aviaLink({ origin, dest, depart, ret, pax = 1, cabin = 'Y' }) {
  const o = (origin || '').toLowerCase(), d = (dest || '').toLowerCase()
  const cls = cabin === 'C' ? 'c' : 'y'   // 비즈니스(C) 선택 시 트립닷컴도 비즈니스로 열기
  const q = `dcity=${o}&acity=${d}&ddate=${depart}${ret ? `&rdate=${ret}` : ''}&triptype=${ret ? 'rt' : 'ow'}&class=${cls}&quantity=${pax || 1}&locale=ko-KR&curr=KRW`
...
function offerLink(o, pax = 1, cabin = 'Y') { return aviaLink({ origin: o.origin, dest: o.destination, depart: (o.departure_at || '').slice(0, 10), ret: (o.return_at || '').slice(0, 10), pax, cabin }) }
const durStr = m => m ? `${Math.floor(m / 60)}시간 ${m % 60}분` : ''

function FlightResult({ o, low, pax = 1, cabin = 'Y' }) {
  const dep = fmtISO(o.departure_at), ret = fmtISO(o.return_at)
  return (
    <a href={offerLink(o, pax, cabin)} target="_blank" rel="noopener" className={'block bg-white rounded-2xl shadow-soft p-3.5 active:scale-[.99] transition' + (low ? ' glow-lowest' : '')}>
...
function Flights() {
  ...
  useEffect(() => {
    ...
    Promise.all(ms.map(m => fetch(`${PROXY}/aviasales/v3/prices_for_dates?` + new URLSearchParams({ origin, destination: dest, departure_at: m, currency: 'krw', market: 'kr', one_way: oneway ? 'true' : 'false', sorting: 'price', limit: '200', unique: 'false', trip_class: cabin === 'C' ? '1' : '0' })).then(r => r.json()).catch(() => ({})))).then(rs => { if (!alive) return; const map = {}; rs.forEach(j => (j.data || []).forEach(o => { const k = (o.departure_at || '').slice(0, 10); if (k && (!map[k] || o.price < map[k])) map[k] = o.price })); setFlexPrices(map) })
    return () => { alive = false }
    ...
    <div className="px-4 pt-2 pb-4 space-y-3">
      ...
      <Sheet open={ovPax} onClose={() => setOvPax(false)} title="인원 · 옵션">
        <div className="px-5 pb-6 pt-1 divide-y divide-slate-100">
          ...
          <div className="pt-3"><button onClick={() => { setOvPax(false); if (st.status === 'route' && !anywhere) routeSearch(dest, day || depDate, retDay || retDate) }} className="w-full bg-brand-500 text-white font-bold rounded-2xl py-3">완료</button></div>
        </div>
      </Sheet>
      ...
      {st.status === 'route' && <>
          ...
          {view.length ? view.map((o, i) => <FlightResult key={i} o={o} low={o.price === lowest} pax={pax} cabin={cabin} />)
          ...
          {(() => {
            ...
            return <>
              <a href={aviaLink({ origin, dest, depart: dd, ret: rr, pax, cabin })} target="_blank" rel="noopener" onClick={() => haptic()}
